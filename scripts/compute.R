...
if (length(args) != 4) {
  stop(
    ...
    "Example:\n",
    "Rscript compute_matches.R data.tsv truth.csv results_ 10"
  )
}
...
# --- write outputs (PREFIX, not directory) ---
matched_drugs_file <- paste0(out_prefix, "matched_drugs.csv")
matched_count_file <- paste0(out_prefix, "matched_count.csv")
